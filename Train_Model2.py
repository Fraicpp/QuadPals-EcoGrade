...
def main():
    ...
    model = YOLO("yolov8n.pt")
        ...
        # Training configuration test 2
    model.train(
        data="C:/Users/turin/Desktop/Opencv/QP.PET-2/data.yaml",  
        epochs=100,  # Reduced to prevent overfitting (200 was too high for a small dataset)
        imgsz=640,  # Kept at 640 for balance between accuracy and speed
        batch=16,  # Adjusted based on GPU memory (reduce to 8 if out-of-memory errors occur)
        workers=4,  # Kept for efficient data loading
        ...
        mosaic=1.0,  # Mosaic augmentation (enabled for better generalization)
        mixup=0.1,  # Reduced Mixup augmentation (from 0.2 to 0.1 to avoid over-augmentation)
        perspective=0.0005,  # Slight perspective transformation
        box=0.05,  # Box loss gain
        cls=0.5,  # Classification loss gain
        dfl=1.0,  # Distribution focal loss gain
        patience=10,  # Early stopping if no improvement (reduced from 20 to 10 for small datasets)
        ...
        optimizer="auto",  # Automatically select the best optimizer (AdamW or SGD)
        dropout=0.2,  # Add dropout for regularization (if supported by your YOLOv8 version)
